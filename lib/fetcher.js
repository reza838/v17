...
.then(json => {
// console.log(json) For JSON file
resolve(json)
})
.catch((err) => {
reject(err)
...
.then(text => {
//console.log(text) From Const Text :)
resolve(text)
})
.catch((err) => {
reject(err)
...
// THIS return pad(hours) + ':' + pad(minutes) + ':' + pad(seconds)
return `${pad(hours)}Jam - ${pad(minutes)}Menit -  ${pad(seconds)}Detik\n\n 「 𝗕𝗔𝗜𝗟𝗘𝗬𝗦 𝗦𝗘𝗟𝗙𝗕𝗢𝗧 」`
}
exports.createExif = (pack, auth) =>{
const code = [0x00,0x00,0x16,0x00,0x00,0x00]
//Exif Const For Pack To Sticker
...
// Func String len = len < 16 ? `0${len.toString(16)}` : len.toString(16)
